...
// RecordSwarmServiceAnnotation records metric information related to pod annotations
func RecordSwarmServiceAnnotation(annotator string, podName string) {
	podsAnnotated.With(prometheus.Labels{"annotator": annotator, "swarm_service_name": podName})
	totalPodsAnnotated.With(prometheus.Labels{"annotator": annotator, "swarm_service_annotated": "total"}).Inc()
}
...
func init() {
	httpResults = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace: "perceptor",
		Subsystem: "docker_perceiver",
		Name:      "http_response_status_codes",
		Help:      "success/failure responses from HTTP requests issued by docker perceiver",
	},
		[]string{"path", "result"})
	...
	durationsHistogram = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			...
			Subsystem: "docker_perceiver",
			Name:      "timings",
			Help:      "time durations of docker perceiver operations",
	...
	errorsCounter = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Subsystem: "docker_perceiver",
		Name:      "errors",
		Help:      "errors from docker perceiver operations",
	...
	podsAnnotated = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Subsystem: "docker_perceiver",
		Name:      "annotations",
		Help:      "individual swarm service annotations",
	}, []string{"annotator", "swarm_service_name"})

	totalPodsAnnotated = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace: "perceptor",
		Subsystem: "docker_perceiver",
		Name:      "total_annotations",
		Help:      "total service annotated",
	}, []string{"annotator", "swarm_service_annotated"})
