...
func makePodAnnotationObj() *bdannotations.BlackDuckPodAnnotation {
	pod := scannedPods[0]
	return bdannotations.NewBlackDuckPodAnnotation(pod.PolicyViolations, pod.Vulnerabilities, pod.OverallStatus, results.HubVersion, results.HubScanClientVersion)
}
...
func TestGetPodContainerMap(t *testing.T) {
	generator := func(obj *bdannotations.BlackDuckImageAnnotation, name string, count int) map[string]string {
