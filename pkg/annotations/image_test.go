...
func TestCreateImageLabelsLongImageNames(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			expected:    map[string]string{"image0": RemoveRegistryInfo(shortImageName)},
		},
		{
		...
		},
		{
			...
			expected:    map[string]string{"image0": RemoveRegistryInfo(longImageName[0:63])},
		},
		{
	...
	for _, tc := range testcases {
		obj := NewImageAnnotationData(2, 10, "NOT_IN_VIOLATION", "http://url/ofthe/hub/scan", "1.1.1", "1.1.1")
		...
		for k, v := range tc.expected {
			if val, ok := result[k]; !ok {
				...
				if len(val) > 63 {
					t.Errorf("[%s] key %s has value %s, which is longer than 63 characters", tc.description, k, val)
				}
			}
		}
		for k := range result {
