...
func (qw *QuayWebhook) Run() {
	...
	if len(qw.certificate) > 0 && len(qw.certificateKey) > 0 {
		...
		if errC != nil || errK != nil {
			...
			log.Infof("Webhook: Starting HTTPs webhook with TLS enabled for quay on :3008 at /webhook")
		...
		log.Infof("Webhook: starting HTTP webhook for quay on :3008 at /webhook")
